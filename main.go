...
func main() {
	...
	for {
		...
		drawBar := func(val float64, color string) string {
			...
			width := 10     // Slightly smaller to fit 6 bars
			percent := val / maxVal
			if percent > 1.0 {
				percent = 1.0
			}
